	...
	var rhinoSlider = function (element, opts) {
		...
		var
			...
			init = function ($slider, settings, vars) {
				...
				settings.showTimeDefault = settings.showTime;
...
//							pause();
			...
			next = function ($slider, settings, $next) {
				...
				// data-wait by PsiTrax
				if(settings.respectItemWait) {
					if(vars.next.attr('data-wait') > 0 && vars.next.attr('data-wait') != settings.showTime)	{
						settings.showTime = vars.next.attr('data-wait');
					} else {
						settings.showTime = settings.showTimeDefault;
					}
				}
